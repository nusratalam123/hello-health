...
export default function HealthSuggestionsPage() {
  ...
  useEffect(() => {
    const fetchHealthData = async () => {
      try {
        ...
        const response = await axios.get(`https://hello-health-backend.vercel.app/api/health-data/single/${userID}`, {
          method: 'GET',
          headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post('https://hello-health-backend.vercel.app/api/health-data/submit', {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}` // Assuming token is stored in localStorage
        },
